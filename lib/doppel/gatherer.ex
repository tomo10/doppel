defmodule Doppel.Gatherer do
  @moduledoc """
  This server that both starts the ball rolling and determines when things have completed. When it does it fetches the results and reports on them
  """
  use GenServer
  @me Gatherer
  ...
  def handle_info(:kickoff, worker_count) do
    1..worker_count
    |> Enum.each(fn _ -> Doppel.WorkerSupervisor.add_worker() end)
  ...
  def handle_cast({:result, path, hash}, worker_count) do
    Doppel.Results.add_hash_for(path, hash)
    {:noreply, worker_count}
  end
  ...
  defp report_results() do
    ...
    Doppel.Results.find_duplicates()
